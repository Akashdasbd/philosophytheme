    ...
    <section class="s-content s-content--narrow s-content--no-padding-bottom">
        <article class="row format-standard">
            ...
            <div class="col-full s-content__main">
                ...
                $philosopy_chargs= array(
                    'post_type'=> 'chapters',
                    'posts_per_page'=> -1,
                    'meta_key'=> 'parent_post',
                    'meta_velue'=> get_the_ID()
                );
                $philosopy_chapters = new WP_Query( $philosopy_chargs );

                echo "<h3>";
                _e("Chpters","philosopy");
                echo "</h3>";
                ...
                wp_reset_query();
                    ...
                    $philosopy_chapter_attached = get_post_meta( get_the_ID(), 'attached_cmb2_attached_posts', true );
                    foreach ( $philosopy_chapter_attached as $attached_post ) {
                        $philosopy_chl =get_permalink($attached_post);
                        $philosopy_cht = get_the_title($attached_post);
                        if (!$philosopy_chapter_attached){
                            echo "no Chapter";
                        }else{
                            printf("<a href='%s'>%s</a><br>",$philosopy_chl,$philosopy_cht);
                        }
